...
## Learning Objectives
 ...
 1. Understand the different data types in R
...
### Vectors 
The most basic data structure in R is called a *vector*. A vector contains multiple elements with the same type. You can assign a vector with the function `c()`. You can have a vector of any `type`. If you attempt to create a vector with more data that has more than one data type, R will coerce all of the elements to same type without warning you.
...
There are several ways to initialize a matrix. The first way is by using the `matrix()` function which 

combining vectors with the functions row-wise or column-wise with `rbind()` and `cbind()` respectively.
...
Just like vectors, all of the data within a matrix must be the same type, so R will coerce all of the elements to the same type.
