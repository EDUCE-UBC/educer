...
```{r dataframe_summary}
my_df <- read_csv("data/Saanich_OTU_metadata.csv")
...
Perhaps a more common and useful way of subsetting a data frame is to return a column from the data frame. The operator to perform this function is `$`.
```{r subsetting_df_2}
new_df$vect1
```
...
*Note*: you can also return a column with square bracket notation
```{r subsetting_df_3}
new_df[,1]
```
