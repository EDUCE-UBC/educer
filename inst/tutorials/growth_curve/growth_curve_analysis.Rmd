...
Use the `filter()` and `min()` functions to remove the first timepoint from the dataset.  Plot the growth curves of non-blank cultures.
...
growth_data_full %>%
  filter(blank == FALSE) %>%
  # filter(Time != min(Time), blank == TRUE) %>%
  ggplot(aes(x=Time, y=net_OD, colour = media)) +
  geom_point() + 
  labs(x = 'Time (h:m:s)', y = 'OD600')
```
...
single_well_data <- growth_data_full %>%
  filter(well == 'A10')
...
```{r test_growth_rates1, exercise=TRUE, exercise.setup = "plotting_growth_curves_setup"}
...
filter(well == 'C2' | well == 'C4'...)
```

```{r test_growth_rates1-solution}
growth_data_full %>%
  filter(Time != min(Time)) %>%
  filter(well == 'C2' | well == 'C4' | well == 'C8' | well == 'C10') %>%
  ggplot(aes(x=Time, y=net_OD, colour = media)) +
  geom_point() + 
  labs(x = 'Time (h:m:s)', y = 'OD600')
```
...
```{r test_growth_rates2, exercise=TRUE, exercise.setup = "plotting_growth_curves_setup"}

```

```{r test_growth_rates2-hint-1}
single_well_data <- growth_data_full %>% # dataframe must be filtered to 1 well
  filter(well == 'C4')
```
...
What is the growth rate of well C4?

```{r C4_check, exercise=TRUE, exercise.setup = "data_reformating_setup"}

```

```{r C4_check-check}
grade_result(
  pass_if(~identical(.result, 0.0001456611))
)
```

What is the growth rate of well C10?

```{r C10_check, exercise=TRUE, exercise.setup = "data_reformating_setup"}

```

```{r C10_check-check}
grade_result(
  pass_if(~identical(.result, 9.485003e-05))
)
```

```{r growth_rates_question, echo=FALSE}
question("Is the relationship between _E. coli_ growth rates in LB and MM as expected?",
  answer("The max growth rate in LB is  more than 1.5 times the max growth rate in MM. We would expect it to be slightly higher.", correct = TRUE),
  answer("The max growth rate in LB is more than 10 times the max growth rate in MM. We would expect it to be higher."),
  answer("The max growth rate in LB is about 1.5 times the max growth rate in MM. This seem incorect, as we would expect it to be slightly lower."),
  answer("We don't have a way of predicting the relationship between growth rates in LB vs. MM"),
  answer("The max growth rate in MM is about 1.5 times the max growth rate in LB This seem incorect, as we would expect it to be slightly lower."),
  random_answer_order = TRUE,
  allow_retry = TRUE
  )
```
