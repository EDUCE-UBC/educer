...
In this tutorial, we will import optical density (OD~600~) data and plot growth curves of the *E. coli* cultures. We will then use the `growthrates` package to calculate maximum growth rates, before determining whether there is a statistical difference between the growth rates of culture in LB and MM.
...
tuesday_raw_data <- read_csv(system.file("resources/data",  "Tuesday_data.csv", package = "educer"), col_names = TRUE, skip = 12)
thursday_raw_data <- read_csv(system.file("resources/data",  "Thursday_data.csv", package = "educer"), col_names = TRUE, skip = 12)
friday_raw_data <- read_csv(system.file("resources/data",  "Friday_data.csv", package = "educer"), col_names = TRUE, skip = 12)
...
tuesday_data <- tuesday_raw_data %>%
  pivot_longer(-c(Time, Temp), names_to = 'well', values_to = 'OD600')

thursday_data <- thursday_raw_data %>%
  pivot_longer(-c(Time, Temp), names_to = 'well', values_to = 'OD600')

friday_data <- friday_raw_data %>%
  pivot_longer(-c(Time, Temp), names_to = 'well', values_to = 'OD600')

tuesday_data_info <- right_join(tuesday_data, well_data, by = 'well') %>%
  filter(!is.na(student), !is.na(Time))

thursday_data_info <- right_join(thursday_data, well_data, by = 'well') %>%
  filter(!is.na(student), !is.na(Time))

friday_data_info <- right_join(friday_data, well_data, by = 'well') %>%
  filter(!is.na(student), !is.na(Time))

tuesday_blank_LB <- calc_blank_EDU(growth_data_info, 'LB')

tuesday_blank_MM <- calc_blank_EDU(growth_data_info, 'MM')

thursday_blank_LB <- calc_blank_EDU(growth_data_info, 'LB')

thursday_blank_MM <- calc_blank_EDU(growth_data_info, 'MM')

friday_blank_LB <- calc_blank_EDU(growth_data_info, 'LB')

friday_blank_MM <- calc_blank_EDU(growth_data_info, 'MM')

tuesday_data_LB <- growth_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - tuesday_blank_LB, Experiment = 'tuesday')

tuesday_data_MM <- growth_data_info %>%
  filter(media == 'MM') %>%
  mutate(net_OD = OD600 - tuesday_blank_MM, Experiment = 'tuesday')

thursday_data_LB <- growth_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - thursday_blank_LB, Experiment = 'thursday')

thursday_data_MM <- growth_data_info %>%
  filter(media == 'MM') %>%
  mutate(net_OD = OD600 - thursday_blank_MM, Experiment = 'thursday')

friday_data_LB <- growth_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - friday_blank_LB, Experiment = 'friday')

friday_data_MM <- growth_data_info %>%
  filter(media == 'MM') %>%
  mutate(net_OD = OD600 - friday_blank_MM, Experiment = 'friday')


growth_data_full <- bind_rows(tuesday_data_LB, tuesday_data_MM, thursday_data_LB, thursday_data_MM, friday_data_LB, friday_data_MM)
```
...
tuesday_blank_LB <- calc_blank_EDU(tuesday_data_info, 'LB')

tuesday_blank_MM <- calc_blank_EDU(tuesday_data_info, 'MM')

thursday_blank_LB <- calc_blank_EDU(thursday_data_info, 'LB')

thursday_blank_MM <- calc_blank_EDU(thursday_data_info, 'MM')

friday_blank_LB <- calc_blank_EDU(friday_data_info, 'LB')

friday_blank_MM <- calc_blank_EDU(friday_data_info, 'MM')

tuesday_data_LB <- tuesday_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - tuesday_blank_LB, Experiment = 'tuesday')

tuesday_data_MM <- tuesday_data_info %>%
  filter(media == 'MM') %>%
  mutate(net_OD = OD600 - tuesday_blank_MM, Experiment = 'tuesday')

thursday_data_LB <- thursday_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - thursday_blank_LB, Experiment = 'thursday')

thursday_data_MM <- thursday_data_info %>%
  filter(media == 'MM') %>%
  mutate(net_OD = OD600 - thursday_blank_MM, Experiment = 'thursday')

friday_data_LB <- friday_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - friday_blank_LB, Experiment = 'friday')

friday_data_MM <- friday_data_info %>%
  filter(media == 'MM') %>%
  mutate(net_OD = OD600 - friday_blank_MM, Experiment = 'friday')
...
calc_growthrates_EDU <- function(dataframe) {
  growth_rates <- dataframe %>%
    distinct(interaction(well, Experiment), .keep_all = TRUE) %>%
    select(c(Experiment, well, student, blank, media)) %>%
    filter(blank == FALSE)
  
  growth_rates$growth_rate <- NA
  for (w in 1:length(growth_rates$well)){
    growth_rate_data <- dataframe %>%
    filter(well == growth_rates$well[w], Experiment == growth_rates$Experiment[w])
...
growth_data_full %>%
  filter(!is.na(student), blank == FALSE, Experiment == 'friday', well == 'C10') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)

growth_data_full %>%
  filter(!is.na(student), blank == TRUE, Experiment == 'friday') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)
```
...
test_data <- growth_data_full %>%
  filter(well == 'B10', Experiment == 'thursday') %>%
  filter(Time != min(.$Time))
...
# growth_rates <- growth_data_full %>%
#   distinct(interaction(well, Experiment), .keep_all = TRUE) %>%
#   select(c(Experiment, well, student, blank, media)) %>%
#   filter(blank == FALSE)

growth_rates <- growth_data_full %>%
  # filter(Experiment != 'friday') %>%
  distinct(interaction(well, Experiment), .keep_all = TRUE) %>%
  select(c(Experiment, well, student, blank, media)) %>%
  filter(blank == FALSE)
...
growth_rates$growth_rate <- NA
for (w in 1:length(growth_rates$well)){
  growth_rate_data <- growth_data_full %>%
    filter(well == growth_rates$well[w], Experiment == growth_rates$Experiment[w])
...
growth_rates %>%
  filter(Experiment == 'tuesday') %>%
  ggplot(aes(x=growth_rate, color = media)) +
  geom_density()

growth_rates %>%
  filter(Experiment == 'thursday') %>%
  ggplot(aes(x=growth_rate, color = media)) +
  geom_density()
...
growth_rates %>%
  ggplot(aes(x=growth_rate, color = interaction(media, Experiment))) +
  geom_density()
```
