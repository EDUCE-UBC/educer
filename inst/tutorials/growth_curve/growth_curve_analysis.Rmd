...
growth_data_full %>%
  filter(!is.na(student), blank == FALSE, Experiment == 'tuesday') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)

growth_data_full %>%
  filter(!is.na(student), blank == TRUE, Experiment == 'tuesday') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)

growth_data_full %>%
  filter(!is.na(student), blank == FALSE, Experiment == 'thursday') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)

growth_data_full %>%
  filter(!is.na(student), blank == TRUE, Experiment == 'thursday') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)

growth_data_full %>%
  filter(!is.na(student), blank == FALSE, Experiment == 'friday') %>%
  ggplot(aes(x=Time, y=net_OD, color = media, group = media)) +
  geom_point(shape=19, size=1)
...
test_data <- growth_data_full %>%
  filter(well == 'E10', Experiment == 'friday') %>%
  filter(Time != min(.$Time))
...
growth_rates %>%
  ggplot(aes(x=media, y = growth_rate, fill  = interaction(media, Experiment))) +
  geom_boxplot()

growth_rates %>%
  filter(Experiment == 'tuesday') %>%
  t_test(growth_rate ~ media, paired = FALSE) %>%
  add_significance
...
growth_rate_ttest <- growth_rates %>%
  t_test(growth_rate ~ media, paired = FALSE) %>%
  add_significance
growth_rate_ttest
```
