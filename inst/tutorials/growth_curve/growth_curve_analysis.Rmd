...
3. Calculating OD~600~ in "blank" wells at T~0~ to generate `blank_**` values for different media types
    + Functions: `calc_blank(<dataframe>, <Media-Type>)`
...
blank_LB <- calc_blank(growth_data_info, 'LB')

blank_MM <- calc_blank(growth_data_info, 'MM')
...
blank_OD <- growth_data_info %>%
  filter(!is.na(Time)) %>%
  filter(blank == TRUE, Time == min(.$Time)) %>%
  group_by(media) %>%
  summarise(blank_OD = mean(OD600))

growth_data_LB <- growth_data_info %>%
  filter(media == 'LB') %>%
  mutate(net_OD = OD600 - blank_OD %>%
           filter(media == 'LB') %>%
           .$blank_OD)
...
Here, we use the `growthrate_model(<fitlered_dataframe>)` and `growthrate_plot(<fitlered_dataframe>)` function to model and plot the maximum growth rate of a single sample.  To do this, first filter your dataframe down to a single `well`.
...
growthrate_model(test_data)

growthrate_plot(test_data)
```
...
growth_rates <- calc_growthrates(growth_data_full)
