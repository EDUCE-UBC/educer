---
title: "Intermediate ggplot tutorial"
...
_ggplot2_ is an add-on package to R. It is an alternative to base graphics that has become very popular, to the point where it is recommended/preferred unless you have old code that already uses a different graphing package.
...
This is where more tidyverse data manipulation can be used. Since we need a single variable to use as x, we need to `gather` our 3 geochemical variables into a single column. We also have to keep (but not gather) our future y-variable, *i.e.* Depth.
```{r}
dat %>%
...
```{r warning=FALSE}
p <- cowplot::plot_grid(p1, p2, labels=c("A", "B"), align="h", axis="tb", rel_widths=c(3/4, 1/4))
