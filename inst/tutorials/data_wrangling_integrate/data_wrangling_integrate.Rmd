...
description: This material will be integrated into other tutorials.
---
...
Since a table is a group of vectors, we can extract a single column (vector) using the `$` operator. 
...
- Oxygen (O~2~) in µM
- Nitrate (NO~3~) in µM
- Hydrogen sulfide (H~2~S) in µM

```{r}
dat <- select(dat,
              Cruise, Date, Depth, Temperature, Mean_O2, NO3, Mean_H2S)
```
...
```{r}
filter(subset_dat, Mean_O2 == 204.259)
```
...
```{r}
filter(subset_dat, Mean_O2 %in% c(40.745, 204.259))
```

`is.na(variable)` returns rows where the variable is `NA` (Not Available).

```{r}
filter(subset_dat, is.na(Mean_O2))
```
...
```{r}
filter(subset_dat, Mean_O2 <= 120 & Mean_O2 >= 20)
```

Logical OR `|`. Find the rows where the value is <= 50 **OR** >= 150.

```{r}
filter(subset_dat, Mean_O2 <= 50 | Mean_O2 >= 150)
```
...
dat %>%
  group_by(Depth) %>%
  summarise(Mean_O2 = mean(Mean_O2, na.rm = TRUE),
            SD_O2 = sd(Mean_O2, na.rm = TRUE))
```
...
```{r summarise_exercise, exercise=TRUE, exercise.lines=5}
pdat <- raw_dat
pdat <- group_by() %>%
  summarise()
