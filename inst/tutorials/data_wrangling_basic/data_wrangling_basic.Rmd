...
write.csv(combined_Data, "Saanich_OTU_metadata.csv", row.names = FALSE )
```
...
```{r saving_quiz, echo = FALSE}
quiz(
  question("How do we make sure the datatable is saved to 'OTU_metadata_table' in our environment?",
           answer("entering 'OTU_metadata_table' in the console displays the table"),
...
```{r view_as_tibble, exercise = TRUE, exercise.lines = 5}
OTU_metadata_table
```
...
```{r dimentions, exercise = TRUE, exercise.lines = 7}
...
```{R select_1, exercise = TRUE, exercise.lines = 5}
...
```{R view, exercise = TRUE, exercise.lines = 3}
restricted_columns
```
...
```{r boolean_exercise,exercise = TRUE, exercise.lines = 5 }
x = 6
y = "cat"
...
```{r boolean_exercise2,exercise = TRUE, exercise.lines = 5 }
...
```{r boolean5, exercise = TRUE, exercise.lines = 5}
...
```{r slice2, exercise = TRUE, exercise.lines = 5}
...
```{r slice3, exercise = TRUE, exercise.lines = 5}
slice(OTU_metadata_table, 1:5)
```

```{r slice_quiz, echo = FALSE}
quiz(
...
Your resulting pdat object should be a [`r dim(temp)`] data frame. The data has been loaded for you into the "dat" variable 

```{r summary_exercise, exercise = TRUE, exercise.lines = 5}
dat <- geochemicals
```
...
Your resulting pdat object should be a [`r dim(temp)`] data frame. *Hint:* Can you filter based on a variable that you previously removed by not selecting it?

```{r summary_exercise2, exercise = TRUE, exercise.lines = 5}
dat <- geochemicals
```
