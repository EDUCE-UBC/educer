...
However, the simplicity of base R graphics may be deceitful: while base R makes it easy to plot a scatter plot or histogram, it is hard to scale it up to more complex graphics.
...
The fundamental idea behind the grammar of graphics is that a graphic consists of some *geometric objects* (e.g., points, lines, bars...) whose *aesthetic attributes* (e.g., size, shape, colour, axes...) encode *data*.

To build a ggplot, you combine different elements (called "layers"), like building blocks. Here are some of ggplot2's building blocks:

* **data:** a data frame where each *variable* is one column 
* **aesthetics:** map variables to visual attributes (e.g., position, color...)
* **geoms:** geometric objects that represent data (e.g., points, bars...)
...
The idea is to independently specify and combine the blocks to create the plot
you want. You need at least the first three building blocks (data, aesthetic map, and geometric objects) to create a plot.

### Your first ggplot: scatterplot
...
To do so, we need to provide our three basic building blocks:

* The first argument of ggplot is the *data*. Hence, we can pipe our data into the ggplot function. 
* The second argument is the *aesthetic map*. This argument is a function whose arguments specify the variables (columns) and how they map onto the plot. In this case, the first variable (oxygen) will map into the x-axis and the second variable (carbon dioxide) will map into the y-axis.
* The final argument is the *geometric object* that represents our data. In this case, we will use `geom_point()` because data is represented as points.

Run the code below to produce your first ggplot:
...
What if we want to include all three variables (`Mean_O2`, `Mean_CO2`, and `Depth`) in the same plot? We ran out of axes after `x` and `y`, but we can use other aesthetic mappings to represent further variables. For instance, we can use the points' colour to represent depth. Run the code below and examine the output:
...
As an alternative to point color, we can also map a continuous variable by using differently sized points. Modify the box below so as to use the `size` aesthetic instead of the `colour` aesthetic:
...
We can also color points by a categorical variable. Recall our `long_dat` data frame:
