...
This section is a reminder of how to manipulate data in base R. If *all* of these concepts are completely new to you, consider doing our "Introduction to R and RStudio" tutorial. If you feel comfortable extracting columns from data frames and subsetting vector elements by index or by logical conditions, feel free to skip ahead to the next section ("Data wrangling").
...
As we saw above, there are a lot of missing data in the `oxygen` vector. Let's say we want to find all `NA` elements. The function `is.na()` returns `TRUE` for each element of the argument (e.g. if the argument is a vector) that is equal to `NA`. In the box below, use this function to print a logical vector that is `TRUE` when `oxygen` is `NA`:
...
You can also use the `sum()` function to count the number of `TRUE`s in a logical vector:

```{r sum_na, exercise = TRUE, exercise.setup = "setup_oxygen"}
sum(is.na(<vector>))
```
...
You can then use the logical vector `not_na` to subset the `oxygen` vector (just like you would use a vector of indices) using the square bracket notation from above. In the box below, let's re-define `oxygen` so as to contain only non-NA values. Then, use `sum()` to prove that there are actually no `NA`s left.
...
#oxygen <- oxygen[not_na] # one possible solution
oxygen <- oxygen[!is.na(oxygen)] # another possible solution
...
In the statements above, `a` and `b` may be single elements (scalars) or they may be vectors:

- When both are scalars, the output will be a logical scalar (i.e. either `TRUE` or `FALSE`).
- When one is a scalar and the other is a vector, each element of the vector is compared to the scalar (so that the output will be a logical vector).
- Finally, when both `a` and `b` are vectors of the same length, elements with the same index are compared with each other, and the output is again a logical vector. You can't compare vectors of different lengths.
...
You can use `rename(<data_frame>, <new_name> = <old_name>)` to assign new names to your columns. 
You can rename as many columns as you want at the same time.
For example, in the box below, we remove the `Mean_` prefix from some of the columns. 
Run it and examine the output. Then, modify it so as to remove the prefix from
the remaining chemicals (CO$_2$, N$_2$O, and CH$_4$).
...
Use `mutate(data_frame, y = function(x))` to apply a transformation to some column `x` and assign the output to the column name `y`. 
The transformation can be a function (e.g., `log(x)`) or a mathematical operation. 
As an example, run the code below to divide `Depth` by 1000 (converting its units from metres to kilometers) and assign the output to a new column, `Depth_km`.
...
You can also facet your plot on the combination of two variables. To do this, add the layer `facet_grid()` instead of `facet_wrap()`. The only other difference is that `vars()` will now take two arguments, one for each variable name. 
...
long_dat %>%
  ...
  geom_point() +
  scale_y_reverse() +
  facet_grid(vars(Cruise, Chemical))
```
