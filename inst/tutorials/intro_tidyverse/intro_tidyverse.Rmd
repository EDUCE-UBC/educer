...
* You complete the graphic by adding one or more layers representing *geometric objects*, or "geoms". These geometric objects represent our data. In this case, we will use `geom_point()` because data is represented as points. There are many geom functions for different types of data visualization, and we will learn some of them in this tutorial.
...
dat %>%
  ...
  geom_smooth()
```
Note: `geom_smooth()` is a convenient and versatile geom. You can use it to quickly plot linear regression models by calling `geom_smooth(method = "lm")`. Try it out in the code box above!
