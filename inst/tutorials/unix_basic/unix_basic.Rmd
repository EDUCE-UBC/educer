...
1.  You can go **up one directory** (called the parent directory) using `cd ..`.
...
When looking at the content of a directory it is often difficult to identify what names indicate files or directories. You can get additional information by using options specifically available for a given command. The simplest option starts with a `-` followed by a single character.

For example, if you want to show all of the file type indicators, you would use `ls -F`.
...
You can learn more about flags or the general use of a command by looking up the **manual** of many commands with `man`. For example, if you look up the manual of `pwd` with `man pwd`, you will see the following.
...
This is a text document with information on every possible modifier of the `pwd` command. There are manuals for many common commands and follow this general layout:
...
+----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Synopsis | How to use the command including options and arguments. Optional parts of the command are enclosed in brackets `[ ]` . parts of the command that can be provided multiple times are indicated by an ellipsis `…` |
+----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|          |                                                                                                                                                                                                                  |
+----------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
|          |                                                                                                                                                                                                                  |
...
Manuals are very comprehensive, large documents that make them often difficult to navigate. `man` actually opens these documents using another program called `less` (see below) and uses the same [keyboard shortcuts](#less) for navigation.

Unfortunately, the way to look up documentation of a given command might differ among the OS platforms. If `man` does not work, try the name of the command followed by `--help`, e.g.

```{bash eval=FALSE}
pwd --help
```

Notice the two dashes `--` instead of a single one that we used for options earlier? Two dashes are used when the following option is given in long form, i.e. as a multi-character word instead of a single letter (the short form). Some commands will offer both a short and a long form of a given option, e.g. `-h` and `--help` both invoke the help for a command.

### `less` {#less}

Manuals are very comprehensive, large documents that make them often difficult to navigate. `man` actually opens these documents using another program called `less` that is used in general to look at plain text files. `less` offers a lot of shortcuts that make is easier to navigate those documents, here selection of commonly used ones:
...
This concludes the main commands used to navigate a computer. Now you *could* end your terminal session by closing the window. However, this is not best practice and could cause issues if you are remotely logged into another machine.
