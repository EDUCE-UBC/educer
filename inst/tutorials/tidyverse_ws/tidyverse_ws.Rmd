...
author: >
  Gil J. B. Henriques, Kim Dill-McFarland and Kris Hong.<br>
  A tutorial by [ECOSCOPE](http://ecoscope.ubc.ca/) at UBC.
date: "version `r format(Sys.time(), '%B %d, %Y')`"
description: >
  In this workshop, we delve into data manipulation using dplyr/tidyr and
  graphics with ggplot in R. We teach different ways to manipulate data in
  tabular and text forms as well as the critical concepts underlying the grammar
  of graphics and how they are implemented in R.
output: 
  learnr::tutorial:
      progressive: true
      allow_skip: true
...
-   ammonium (NH$_4$) concentration, `Mean_NH4`
-   nitrogen dioxide (NO$_2$) concentration, `Mean_NO2`
-   hydrogen sulfide (H$_2$S) concentration, `Mean_H2S`
-   dinitrogen (N$_2$) concentration, `Mean_N2`
-   oxygen (O$_2$) concentration, `Mean_O2`
-   carbon dioxide (CO$_2$) concentration, `Mean_CO2`
-   nitrous oxide (N$_2$O) concentration, `Mean_N2O`
-   methane (CH$_4$) concentration, `Mean_CH4`
...
3.  filter the new data so as to retain data on Cruise 72 where Depth is smaller than or equal to 20 m.
...
dat
#arrange(dat, Mean_CH4)
```

To sort rows in descending order, use `arrange(<data_frame>, desc(<x>))`. In the box below, arrange `dat` in descending order of `Mean_O2`:
...
mutate(dat, Depth_km = Depth/1000)
```
...
3.  Keep August observations where oxygen data are non-missing using `filter()`, `month()`, and `!is.na()`
...
In the box below, rewrite your code from the previous exercise using pipes. **Tip:** In RStudio, you can use the keyboard shortcut <kbd>Cmd</kbd>+<kbd>Shift</kbd>+<kbd>M</kbd> to enter the pipe operator.
...
In the chain above, the function `n()` returns the number of rows corresponding to each depth and the argument `na.rm = TRUE` inside functions like `mean()` and `sd()` ensures that `NA` values are ignored when calculating those summary statistics.
...
We can undo this operation with the opposite function, `pivot_wider()`, which works similarly to its counterpart. The `pivot_wider()` function takes the following arguments:

-   The data frame to be pivoted;
-   A vector containing the columns that we want to pivot **or** the columns that we do *not* want to pivot (indicated by an exclamation point `!`). If this argument is left blank, it defaults to all columns in the data frame except for those specified in `names_from` and `values_from`;
-   `names_from`: The name of the column whose elements provide the names of the output columns;
-   `values_from`: The name of the column from which we get the values of the output columns.
...
There is a lot to learn about ggplot2, and this tutorial will not cover all of it. If you want to learn more, be sure to read chapters 3 and 28 of Hadley Wickham's book [R for Data Science](https://r4ds.had.co.nz/). For more in-depth information, check out Hadley Wickham's book [ggplot2: Elegant Graphics for Data Analysis](https://ggplot2-book.org/). For a collection of recipes to solve common graphics problems, read Winson Chang's [The R Graphics Cookbook](https://r-graphics.org/). All of these books are available online for free.
...
You can also facet your plot on the combination of two variables. To do this, add the layer `facet_grid()` instead of `facet_wrap()`. The only other difference is that we now need to provide two instances of `vars()`, one for each variable name.
...
long_dat %>%
  ...
  geom_point() +
  scale_y_reverse() +
  facet_grid(vars(Cruise), vars(Chemical))
```
...
The code below produces our box plot. Its x- and y-axis labels already look good, but let's add a title ("Effect of depth"). We can also modify the label of the `fill` aesthetic (currently titled "`Depth < 100`") using the same function. Let's call it "Depth" instead.
...
By default, ggplot uses a characteristic gray colour scheme. We can chose to use a "classic" black and white theme and add it to a ggplot object. There are also many [pre-made themes](http://ggplot2.tidyverse.org/reference/ggtheme.html), and with some experience you can also learn to create your own themes.
...
cowplot::plot_grid(pA, pB, ncol = 1, labels = c("A", "B"), rel_heights = c(1.5, 1))
```

Do your work in the code chunk below:

```{r, final_exercise, exercise = TRUE,  exercise.lines = 20, exercise.setup = "setup_long_dat"}
...
plot_grid(pA, pB, ncol = 1, labels = c("A", "B"), rel_heights = c(1.5, 1))
```
