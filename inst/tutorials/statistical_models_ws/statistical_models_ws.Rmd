...
library(<package>) # not installed on this machine
<function>(gapminder)
```

```{r gapminder-package-solution}
library(gapminder) # Data from Gapminder # Data from Gapminder # Data from Gapminder # Data from Gapminder # Data from Gapminder # Data from Gapminder # Data from Gapminder # Data from Gapminder # Data from Gapminder
head(gapminder)
```
...
Let's use a transformed predictor for creating a new (and hopefully better) model. There are two ways. We can include the function directly into the formula (similar to the last plot) and fit the models. 

Create a model called `lifeExp_model_log_a`. Use the `log()` fucntion to transform the `gdpPercap` variable. 
```{r log_ex, exercise = TRUE}
<object_name> <- lm(lifeExp ~ year + <transform_function>(gdpPercap),
                     data = gapminder)
...
Create a model called `lifeExp_model_poly_a` with the predictor variable transformed via the `poly()` function. 
...
```{r poly_ex-solution}
lifeExp_model_poly_a <- lm(lifeExp ~ year + poly(gdpPercap),
                     data = gapminder)

summary(lifeExp_model_poly_a)
```

Or explicitly write a polynomial equation for one of our predictor.
...
**Note:** We must use `I(gdpPercap^2)` instread of `(gdpPercap^2)` because the symbols `^`, `*`, and `:` have a particular meaning in a linear model. As we saw in ANOVA, these symbols are used to specify *interactions* between predicting variables. 
...
So far, our models for life expectancy have built upon continuous (or discrete but incremental) variables variables. However, we may wonder if being in one continent rather than another has a differential effect on the correlation between `year` and `lifeExp`. 
...
Create a new model called `lifeExp_ancoca`. Use the special character `*` to model the effects of year, continent, and their interaction on life expectancy. Then, use the ANOVA function `aov()` ont he model. 
...
lifeExp_ancoca <- lm(lifeExp ~ year*continent,
               data = gapminder)

summary(aov(lifeExp_ancoca))
```

Based on these results, it really seems that continent should have a role in the model. However, it is not always like this. Let's take a closer loot at Europe and Oceania. 
...
Let's take a look at what the data looks like (with a tad bit of vertical jitter).Using the `esoph` data set, create a scatter plot with the following aesthetics: 
...
It seems like each age group has a different relationship. We should then fit regression lines for each group separately. Using the `p` object we just created, fit a best fit line in the plot vai the`geom_smooth` function. 
...
Each group has very few observations which make the regression less powerful. 
...
The $\beta$ terms are called the *fixed effects*, and the $b$ terms are called the *random effects*. Since the model has both types of effects, it's said to to be a *mixed* model - hence the nameof "LME".
...
# download the lme4 package if you need to
...
#### **Exercise: LME cont.**
What is the intercept and slope of subject #310 from question 1? 
...
# download the car package if you need to
...
# download the lsmeans package if you need to
...
ucb_summary <- summary(<model_object>, by = "Dept")
...
# download the contrast package if you need to
if (!require("contrast")) install.packages("contrast")

ucb_contrast <- contrast::contrast(ucb_model_sum, "pairwise", by = "Dept")
