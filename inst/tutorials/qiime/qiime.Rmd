...
You can now open the current working directory in the file browser on your local machine by entering `open .` on macOS or `explorer.exe .` on WSL. You should now see the `demux.qzv` file in your file browser and drag it onto the [QIIME2 view website](https://view.qiime2.org/) to view it.
...
qiime feature-classifier classify-sklearn \
  --i-classifier /mnt/datasets/project_1/moving_pictures/gg-13-8-99-515-806-nb-classifier.qza \
...
Fiannly, you will often take the data generated in QIIME2 and export it for analysis with other tools such as R.
...
Open file in a text editor on server
...
You will continue to work with `table-with-taxonomy.biom`, `tree.nwk`, and the original `sample-metadata.tsv` file. Copy them to your local computer with `scp`.
