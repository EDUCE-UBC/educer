...
raw_dat <- read_csv("../../MICB301/tutorials/4.MICB301_stats_extend_data.csv")
```
...
```{r}
dat <- mutate(raw_dat, Depth_m = as.factor(Depth_m),
...
Recall the basic `tidyverse` verb syntax:
...
Piping works nicely to condense code and to improve readability. `tidyverse` syntax is very easy to read. Starting with the file, we can load the data frame, select the Cruise, Depth, and oxygen variables, convert Depth and Cruise to factors and filter for only specific values of those variables, all in a single step
```{r message=FALSE}
dat <- read_csv("../../MICB301/tutorials/4.MICB301_stats_extend_data.csv") %>%
  mutate(Depth_m = as.factor(Depth_m),
         Cruise = as.factor(Cruise)) %>%
  filter(Depth_m %in% c(10, 100, 200),
         Cruise %in% c(18, 30, 42, 54, 66, 80, 92))
```
...
- data: 2D table of _variables_
- _aesthetics_: map variables to visual attributes, always contained in `aes()`
- _geoms_: graphical representation of data (points, lines, etc.)
- _stats_: statistical transformations (binning, summarizing, smoothing)
- _scales_: control _how_ to map a variable to an aesthetic
- _guides_: axes, legend, etc.
...
```{r}
ggplot(dat, aes(x = Depth_m, y = O2_uM))
```
...
```{r}
ggplot(dat) +
  geom_point(aes(x = Depth_m, y = O2_uM))
```
...
We can summarize the multiple data points as a boxplot:
```{r}
ggplot(dat, aes(x = Depth_m, y = O2_uM)) +
  geom_boxplot() +
  geom_point(alpha = 0.5) +
...
ggplot(dat, aes(x = Depth_m, y = CH4_nM, fill = Cruise))+
